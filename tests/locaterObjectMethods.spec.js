...
test.describe('Test Group', () => { 
  ...
  test('Check() checks the radio buttons and checkboxes if they havnt been checked yet', async ({ page }) => {
    let checkboxesLink = page.locator("text='Checkboxes'"); // locating by text
    await checkboxesLink.click();
    let checkBox1 = page.locator("#box1"); // locating by css
    await checkBox1.check(); // will check the unchecked box. If the box is checked it will NOT uncheck it.
  ...
  test('unchecked() unchecks the radio buttons and checkboxes if they havnt been unchecked yet', async ({ page }) => {
    ...
    await checkBox2.uncheck(); // will uncheck the checked box.
  });
  ...
  test('selectOption() used for dropdowns', async ({ page }) => {
   ...
   // await simpleDropDown.selectOption( { label: "Option 1" } ); // select by visible text
  ...
  test('innerText() retrives the visible text', async ({ page }) => {
     ...
     let actualText = await headerElement.innerText(); // will return the visible text
     console.log(actualText);
  });

   test('inputValue(): only works with <input>, <textarea>, <select>', async ({ page }) => {
        let inputsLink = page.getByText("Inputs"); // locating the lement by text
      ...
      //  await page.waitForTimeout(3000);
        ...
        // await page.waitForTimeout(3000);
        let inputValue = await inputBox.inputValue(); // only works with <input>, <textarea>, <select>
